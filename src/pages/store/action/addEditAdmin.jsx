...
const StoreAddEdit = props => {
	const [form] = Form.useForm();
	const isVerify = props.location.search.substr(8)
	console.log("🚀 ~ file: addEdit.jsx:32 ~ StoreAddEdit ~ isVerify:", isVerify)
	...
	useEffect(() => {
		...
		if (idAdd) {
			...
			let stores = JSON.parse(sessionStorage.getItem("storesData"));
			console.log("🚀 ~ file: addEdit.jsx:57 ~ useEffect ~ stores:", stores.cuisine)



			if (stores.open_time) {
				stores.open_time = moment(stores.open_time);
			}
			if (stores.close_time) {
				stores.close_time = moment(stores.close_time);
			}
			...
			if (stores.visiting_days) {
				let arr = stores.visiting_days
				// let arr = stores.visiting_days.split(",");
...
return;
		...
		if (props.match.params.id) {
			...
			result = await dispatch({ type: 'stores/editAdmin', payload: formData , });
		}
		else {
			result = await dispatch({ type: 'stores/addAdmin', payload: formData });
		}
