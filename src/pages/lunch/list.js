...
class LunchList extends React.Component {
  constructor(props) {
    ...
    setTimeout(() => document.title = 'Lunch Hall List', 100);
  }
  componentDidMount() {
    this.ListFun();
    this.props.dispatch({ type: 'lunches/clearAction' });
  }
...
if (user.role === "ADMIN") {
  ...
  this.props.dispatch({ type: 'lunches/lunchAllList', payload: searchval, });
} else {
  let search = 'page=' + (this.state.current - 1) + "&limit=" + this.state.limit + "&inactive=" + this.state.inactive + "&searchText=" + this.state.searchText + "&sortBy=" + this.state.sortBy;
  localStorage.setItem('newsSearch', JSON.stringify(this.state))
...
paginationFun = (val) => this.setState({ current: val.current }, () => this.ListFun());
  ...
  searchVal = (val) => {
    ...
    const resultAutos = this.props.lunches.list.data.filter((auto) => auto.hall_name.toLowerCase().includes(val.toLowerCase()))
  ...
  deleteCat = id => {
		this.props.dispatch({ type: 'lunches/lunchDelete', payload: { _id: id }, });
  }
  
  getSnapshotBeforeUpdate(prevProps, prevState) {

		if (this.props.lunches.del && this.props.lunches.del.count > this.state.count && this.props.lunches.del.status) {
			this.setState({ count: this.props.lunches.del.count })
			return true
		}
		if (this.props.lunches.add && this.props.lunches.add.count > this.state.Addcount && this.props.lunches.add.status) {
			this.setState({ Addcount: this.props.lunches.add.count, addModel: false })
			return true
		}
		return null;
	}
  ...
  render() {
    ...
    if (this.state.searchText == '') {
      this.state.listData = lunches.list ? lunches.list.data : [];
    }
    ...
    const productsAvilabilityColumns = [
      {
        title: <strong>Holl Name</strong>,
        dataIndex: 'hall_name',
        render:(val,data) => {
          return (data.hall_name? <span className="text-info cursor-pointer	" >{data.hall_name}</span>:'-')
        }
      },
      {
      ...
      },
      {
        ...
        render: (val, data) => <div onClick={e => e.stopPropagation()}>
          <Popconfirm title="Are you sure delete lunches?" onConfirm={e => { this.deleteCat(data._id); e.stopPropagation() }} okText="Yes" cancelText="No" >
      ...
      <Card>
        ...
        <Table
          ...
          onRow={(record, rowIndex) => {
            return {
              onClick: lunches => this.setState({ addModel: true, detail: record })
            };
          }}
          pagination={{
								position: ['bottomLeft'],
...
export default connect(({ lunches, loading }) => ({
  lunches, 
  loading
}))(LunchList);
